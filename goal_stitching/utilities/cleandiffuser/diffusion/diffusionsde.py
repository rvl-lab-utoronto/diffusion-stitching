...
from ..nn_condition import BaseNNCondition
...
class DiscreteDiffusionSDE(BaseDiffusionSDE):
    ...
    def sample(
        ...
        # which pixel to look at in the sequence for analyzing reverse diffusion gradient stuff
        grad_pixel_index = 256
        # ===================== Initialization =====================
        log = {
            "sample_history": np.empty((n_samples, sample_steps + 1, *prior.shape)) if preserve_history else None,
            "grad_history": np.empty((n_samples, sample_steps + 1, *prior.shape)) if preserve_history else None,  }
        ...
        for i in reversed(loop_steps):
            ...
            # wacky stuff to show locality 
            if preserve_history:
                pred[0,grad_pixel_index,0].backward(retain_graph=True)
                in_grad = xt.grad
                #print(type(in_grad))
                log["grad_history"][:, sample_steps - i] = in_grad.cpu().numpy()
                xt.grad.data.zero_()
